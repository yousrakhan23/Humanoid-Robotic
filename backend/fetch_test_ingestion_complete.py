...
def save_points_to_file(points: List[Dict[str, Any]], filename: str = "test_ingestion_complete.json") -> None:
    """
    Save the fetched points to a JSON file.

    Args:
        points: List of point data dictionaries
        filename: Output filename (default: "test_ingestion_complete.json")
    """
    try:
...
def main():
    ...
    try:
        ...
        try:
            ...
            for col in collections:
                try:
                    ...
                    print(f"  - {col.name} (info unavailable)")
            return
        ...
        if save_choice == 'y':
            filename = input("Enter filename (default: test_ingestion_complete.json): ").strip()
            if not filename:
                filename = "test_ingestion_complete.json"
